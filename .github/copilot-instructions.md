...
Write comments, error messages, title of test cases and etc in the code in English.
...
Mocks should generally be generated by `mockgen` in a separate package at the same level as the package defining the interface.
Generate them in a file named `gen.go` and execute `mockgen` with `go:generate`.
Note that mockgen requires the `gomock` package: "go.uber.org/mock/gomock".
Example (directory structure):

```
gogh/
├── core/                # Core functionality - the essence of the program
│   ├── gen.go           # File for generating Mocks
│   ├── repository/      # Definitions related to repositories
│   ├── repository_mock/ # Mocks related to repositories
│   └── ...
...
```
