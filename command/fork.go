...
func Fork(ctx gogh.Context, gitClient *GitClient, hubClient *hub.HubClient, update, withSSH, shallow bool, organization string, repo *gogh.Repo) error {
	...
	remotes, err := gitClient.GetRemotes(ctx, project)
	if err != nil {
		return err
	}
	for name := range remotes {
		if name == newRepo.Owner(ctx) || name == repo.Owner(ctx) || name == "origin" {
			if err := gitClient.RemoveRemote(ctx, project, name); err != nil {
				return err
			}
		}
	}
	if err := gitClient.AddRemote(ctx, project, repo.Owner(ctx), repo.URL(ctx, withSSH)); err != nil {
		return err
	}
	if err := gitClient.AddRemote(ctx, project, newRepo.Owner(ctx), newRepo.URL(ctx, withSSH)); err != nil {
		return err
	}
