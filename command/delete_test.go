...
func TestDelete(t *testing.T) {
	...
	t.Run("delete proj2 explicitly", func(t *testing.T) {
		assert.NoError(t, command.Delete(svc.ev, false, "gogh-test-2"))
		var err error
		_, err = os.Stat(proj1)
	...
	t.Run("delete proj3 with fuzzy", func(t *testing.T) {
		...
		assert.NoError(t, command.Delete(svc.ev, false, "3"))
		var err error
		_, err = os.Stat(proj1)
		assert.NoError(t, err)
		_, err = os.Stat(proj2)
	...
	t.Run("delete proj1 in primary", func(t *testing.T) {
		...
		assert.NoError(t, command.Delete(svc.ev, true, "test"))
		var err error
		_, err = os.Stat(proj1)
		assert.True(t, os.IsNotExist(err))
		require.NoError(t, os.MkdirAll(proj1, 0755))
	...
	t.Run("did not match", func(t *testing.T) {
		assert.EqualError(t, command.Delete(svc.ev, true, "foobar"), "any projects did not matched for \"foobar\"")
		var err error
		_, err = os.Stat(proj1)
		assert.NoError(t, err)
		_, err = os.Stat(proj2)
